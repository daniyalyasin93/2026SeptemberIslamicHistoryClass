# -*- coding: utf-8 -*-
"""Build S04.pptx — evening 4: al-Buṭāḥ, the road to al-Yamāma, and the last front.
...
FACE_TITLE = {
    ...
    "RCT/E-RC63": "Excused, and sent to al-Yamama",
...
FACE_WHEN = {
    ...
    "RCT/E-RC64": "11 AH, before Khalid ؓ arrived",
...
MAP_BEFORE = {
    "ATA/E-TB18": ("Sajah comes down from the north", "s04-01-sajah.json", "1–3",
                   [("al-Jazira", "where she came from"), ("al-Yamama", "where she turned back")]),
    "RCT/E-RC64": ("al-Yamama before the army came", "s04-02-al-yamama-before.json", "1–2",
                   [("Hajr", "the town"), ("Thumama ؓ", "fighting Musaylima already")]),
    "RCT/E-RC44": ("From Buzakha to al-Butah", "s04-03-buzakha-to-al-butah.json", "1–2",
    ...
    "RCT/E-RC63": ("Out again, to al-Yamama", "s04-05-medina-to-al-yamama.json", "1–2",
                   [("Medina → al-Yamama", "the army, and the reserve behind it")]),
    ...
    "RCT/E-RC54": ("Aqraba, and the ground behind it", "s04-07-aqraba.json", "1–2",
                   [("Aqraba", "the enemy camp"), ("the farmland", "behind their own line")]),
    "RCT/E-RC19": ("The line re-formed", "s04-07-aqraba.json", "3–4",
                   [("Muhajirun · Ansar · the tribes", "each under its own banner")]),
    "TSY/E-YK07": ("Hadramawt and Kinda", "s04-08-hadramawt-and-kinda.json", "1",
                   [("Hadramawt", "Ziyad b. Labid ؓ"), ("Kinda", "the zakat camels")]),
    "TSY/E-YK10": ("Round their fires", "s04-08-hadramawt-and-kinda.json", "2–4",
                   [("Kinda", "the four kings")]),
    "TSY/E-YK11": ("The march on al-Nujayr", "s04-09-the-march-on-al-nujayr.json", "1–2",
                   [("al-Muhajir ؓ", "ahead, from Sanaa"), ("Ikrima ؓ", "the main body, behind him")]),
    "TSY/E-YK14": ("To Medina", "s04-10-al-ashath-to-medina.json", "1",
                   [("al-Ashath", "bound, to Abu Bakr ؓ")]),
    "KTK/E-KD01": ("Kinda before Islam", "s04-11-kinda-before-islam.json", "1–2",
...
LESSON_B = ["RCT/E-RC50", "RCT/E-RC63", "RCT/E-RC54", "RCT/E-RC16"]      # STOP B — THE PLANNED END
LESSON_C = ["RCT/E-RC54", "RCT/E-RC58", "RCT/E-RC59", "RCT/E-RC19"]      # STOP C — the day begins
LESSON_D = ["RCT/E-RC50", "RCT/E-RC58", "TSY/E-YK19", "POT/E-PG41"]      # STOP D — the whole ridda map
...
def build():
    ...
    prs = D.deck()
    D.title_slide(prs, "al-Butah, al-Yamama, and the last front", "10–12 AH", "Evening 4")
    ...
    D.note(s, "BOOKEND IN — or paste evening 3's STOP B Line slide from S03.pptx, unchanged "
              ...
              "had never stopped. Tonight we go back to Khālid ؓ, who we last saw at Buzākha — and at "
              "the end of the evening we come back south, to the one province still grey on your map.\n"
    ...
    # STOP B closes the evening as planned; A, C and D are built too, hidden, for another stopping point
    closing(prs, pool, "line_s04_stop_b.png", "s04-close-stop-b.json",
            ...
            "is saved for him.",
            LESSON_B, [("al-Yamama", "the armies in position"), ("Najd", "settled behind them")],
            stop="STOP B")
    closing(prs, pool, "line_s04_stop_a.png", "s04-close-stop-a.json",
            "Khalid ؓ keeps his command, and the last claimant is waiting in al-Yamama with forty "
            "thousand men. What happens when the army reaches him?",
            LESSON_A, [("Najd", "blue"), ("al-Yamama", "still red")],
            hidden=True, stop="STOP A")
    closing(prs, pool, "line_s04_stop_c.png", "s04-close-stop-c.json",
            "The line has been re-formed and the day is not yet decided. Where does an army go when it "
            "cannot retreat?",
            LESSON_C, [("Aqraba", "the day undecided")],
            hidden=True, stop="STOP C")
    closing(prs, pool, "line_s04_stop_d.png", "s04-close-stop-d.json",
            "Every province on the map is blue again. One garden in al-Yamama is still to be told — and "
            "what came out of it changed how we hold the Qur'an. How?",
            LESSON_D, [("the peninsula", "blue"), ("al-Yamama", "one scene still to tell")],
    ...
    print("   %d of %d runsheet cards became slides · %d slides in all (12 hidden: the STOP A, C and D "
